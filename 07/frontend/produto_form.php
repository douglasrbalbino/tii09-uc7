...
if (isset($_GET['id'])) {
    $produto = $dao->getById($_GET['id']); // GET para puxar o id com metodo GET
}
...
if($_POST) {
    ...
    $dataDeValidade = $_POST['dataDeValidade'] ? : null; // Por que é ?:
...
<body>
    ...
    <form action="./produto_form.php" method="post">
        <?php if($produto): ?>
            <input hidden name="id" required value="<?= $produto->getId() ?>"> </br>
        <?php endif; ?>
